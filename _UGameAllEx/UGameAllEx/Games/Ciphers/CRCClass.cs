...
{
    static uint[] crc32_tab = new uint[]{
	...
	public static string GetCRC(byte[] buf)
	{
		uint crc = crc32 (buf, buf.Length);
		return Convert.ToString(crc, 16).ToUpper();
	}

	static public string GetCRCContent(string cont)
	{
		if(string.IsNullOrEmpty(cont)) return "";
		byte[] buff = System.Text.Encoding.UTF8.GetBytes(cont);
		return GetCRC(buff);
	}
}
